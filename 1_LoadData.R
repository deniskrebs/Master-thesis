...
g <- graph_from_edgelist(edgelist, directed = TRUE)
ecount(g)
deg=igraph::degree(g, mode="total")
nodes$degree=deg
...
g.simp <- igraph::simplify(g, remove.multiple = TRUE, remove.loops = TRUE)
...
g.simp.un <- igraph::simplify(g.un, remove.multiple = TRUE, remove.loops = TRUE)
...
plot(g.simp, vertex.size=3, edge.arrow.size=0.02, vertex.label.font=1, vertex.label.cex = 0.01)
